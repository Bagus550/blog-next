...
// Perhatiin baris ini, kita bungkus params-nya jadi Promise
export default async function BlogDetail({ params }: { params: Promise<{ id: string }> }) {
  
  // 1. WAJIB DI-AWAIT dulu di Next.js versi terbaru bray!
  const { id } = await params;

  // 2. Baru deh pake variabel 'id' buat nyari di Supabase
  const { data: post, error } = await supabase
    .from("posts")
    .select("*")
    .eq("id", id) // <--- Pake 'id' yang udah di-await
    .single();
  ...
  return (
    <main className="min-h-screen bg-white text-black p-6 md:p-20 font-sans">
      <div className="max-w-3xl mx-auto">
        <Link href="/" className="text-gray-400 hover:text-black font-bold mb-10 block transition-colors">
          ← Kembali ke Beranda
        </Link>
        ...
        {post.image_url && (
          <img
            src={post.image_url}
            alt={post.title}
            className="w-full h-100 object-cover rounded-[2.5rem] mb-10 shadow-2xl"
          />
        )}
